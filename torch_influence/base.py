...
class BaseInfluenceModule(abc.ABC):
    ...
    def stest(self, test_idxs: List[int]) -> torch.Tensor:
        ...
        vec = self.test_loss_grad(test_idxs)
        print('time of calculating test loss grad: ', round(time.time() - start_time, 2))
        
        return self.inverse_hvp(vec)
    ...
    def influences(
        ...
        # stest is to calculate the iHVP
        # hessian: train loss and entire train data (expected)
        # grad: test loss on the entire test data (expected)
        
        start_time = time.time()
        stest = self.stest(test_idxs) if (stest is None) else stest.to(self.device)
        print('time of calculating ihvp: ', round(time.time() - start_time, 2))
        
        scores = []
        start_time = time.time()
        # gradient of the train loss
        for grad_z, _ in tqdm(self._loss_grad_loader_wrapper(batch_size=1, subset=train_idxs, train=True), total=len(train_idxs)):
            s = grad_z @ stest
            scores.append(s)
        print('time of calculating scores: ', round(time.time() - start_time,2))
    ...
    def _train_jacobian(self, train_idxs):
        """
        pack the jacobian of on the train set into a matrix (N * p, d) 
        where N is the number of train data, p is the number of output, and d is the number of parameters
        """
        ...
        for batch, batch_size in tqdm(self._loader_wrapper(batch_size=1, subset=train_idxs, train=True), total=len(train_idxs)):
            def out_f(theta_):
                """
                map from the parameters to the outputs
                """
                self._model_reinsert_params(self._reshape_like_params(theta_))
                return self.objective.train_outputs(self.model, batch)
            
            J.append(torch.autograd.functional.jacobian(out_f, flat_params))
